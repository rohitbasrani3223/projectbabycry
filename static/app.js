...
function renderChart(metric) {
  ...
  trainChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: {
          labels: { color: '#aaa', font: { family: 'Inter', size: 12 } }
        }
      },
      scales: {
        x: {
          ...
          title: { display: true, text: 'Epoch', color: '#555', font: { family: 'Inter' } }
        },
        y: {
          grid:  { color: 'rgba(255,255,255,0.05)' },
          ticks: { color: '#666', font: { family: 'Inter' } },
...
function showResult(data) {
  ...
  document.getElementById('resultTitle').textContent       = (data.prediction ?? '—').replace('_', ' ');
  ...
  sorted.forEach(([cls, pct]) => {
    ...
    label.textContent = cls.replace('_', ' ');
